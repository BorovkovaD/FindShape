...
Mat DetectorCanny::start()
{
    ...
    GaussianFilterOperator (1, img_gray, img_gaus);
...
void DetectorCanny::SetRGB(int row, int col, TRGB& setPix, Mat& output)
{
...
void DetectorCanny::GetRGB (int row, int col, TRGB& retPix, Mat& input)
{
    ...
    for(i = 1; i < height-1; i++)
        for(j = 1; j < width-1; j++)
        {
            ...
            if(p.r>0)
            {
                 ...
                 * иначе подавляем его*/
                GetRGB( i-1 , j-1 , p , input);
                if(p.r>0)
                {
                    GetRGB( i , j , pix , input);
                    SetRGB(i,j,pix,output);
                    continue;
                }
                GetRGB( i-1 , j , p , input);
                if(p.r>0)
                {
                    GetRGB( i , j , pix , input);
                    SetRGB(i,j,pix,output);
